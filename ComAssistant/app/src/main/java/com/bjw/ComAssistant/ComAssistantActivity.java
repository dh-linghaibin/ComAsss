...
public class ComAssistantActivity extends Activity {
	...
	Button ButtonClear,ButtonSendCOMA,ButtonSendCOMB,ButtonSendCOMC,ButtonSendCOMD,ButtonSendCOMAHex,ButtonSendCOMAReset,ButtonSendCOMACheck;
...
//			String versionCode = String.valueOf(pinfo.versionCode);
		...
		ButtonSendCOMAHex=(Button)findViewById(R.id.ButtonSendCOMAHex);
		ButtonSendCOMAReset = (Button)findViewById(R.id.ButtonSendReset);
		ButtonSendCOMACheck = (Button)findViewById(R.id.ButtonSendCheck);
		...
		ButtonSendCOMAHex.setOnClickListener(new ButtonClickEvent());
		ButtonSendCOMAReset.setOnClickListener(new ButtonClickEvent());
		ButtonSendCOMACheck.setOnClickListener(new ButtonClickEvent());
		...
		for (int i = 0; i < entryValues.length; i++) {
			if(entryValues[i].equals("/dev/ttyGS0")) {
				Log.d("lhb","不出现");
			} else if(entryValues[i].equals("/dev/ttyGS1")) {
				Log.d("lhb","不出现");
			} else if(entryValues[i].equals("/dev/ttyGS2")) {
				Log.d("lhb","不出现");
			} else if(entryValues[i].equals("/dev/ttyGS3")) {
				Log.d("lhb","不出现");
			} else if(entryValues[i].equals("/dev/ttymxc0")) {
				Log.d("lhb","不出现");
			} else {
				allDevices.add(entryValues[i]);
				Log.d("lhb", entryValues[i]);
			}
		}
		Log.d("lhb--", allDevices.toString());
	...
	class ButtonClickEvent implements View.OnClickListener {
		public void onClick(View v)
		{
			if(toggleButtonCOMA.isChecked()) {
				...
				} else if (v == ButtonSendCOMA) {//关门
					//sendPortData(ComA, editTextCOMA.getText().toString());
					String lader = "3a0200000000020a";
					sendPortDataHex(ComA, lader);
			...
			}*/ else if (v == ButtonSendCOMAHex) {//开门
					String lader = "3a0100000000010a";
					sendPortDataHex(ComA, lader);
				} else if (v == ButtonSendCOMACheck) {//检查位置
					String lader = "3a0400000000040a";
					sendPortDataHex(ComA, lader);
				} else if (v == ButtonSendCOMAReset) {//复位盒子
					String lader = "3a0300000000030a";
					sendPortDataHex(ComA, lader);
				}
			} else {
				ShowMessage("请先打开串口:选择串口1-4");
			}
		}
	}
	//----------------------------------------------------打开关闭串口
...
//		}
	...
	private void DispRecData(ComBean ComRecData){
		...
		if ((iRecLines > 100) && (checkBoxAutoClear.isChecked()))//达到500项自动清除
		{
			editTextRecDisp.setText("");
	...
	//----------------------------------------------------串口发送Hex
	private void sendPortDataHex(SerialHelper ComPort,String sOut){
		if (ComPort!=null && ComPort.isOpen())
		{
			ComPort.sendHex(sOut);
		}
